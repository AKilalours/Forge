...
| 6 | Inference frameworks (vLLM, ONNX Runtime) | `generation/generators/base.py`: two-pass scheduler holding one engine at a time, GPU preflight, explicit allocator teardown, scheduler invariants under test. Single GPU. On the serving side, `scripts/export_onnx.py` exports the deployed arm to ONNX Runtime and measures it against the PyTorch path on the same windows: **numerically identical (max delta 2.3e-10, zero verdict flips) and not faster (1.02x to 1.05x at 1 to 4 threads, 0.94x at 10)**. int8, which is where the speedup would have been, fails in onnxruntime's shape inference on a dynamo-exported graph. | 2 and 7 | **implemented and measured; the ONNX result is a negative one and is published as such** |
...
| 7b | Mining the internet at scale | `ingestion/commoncrawl.py` streams Common Crawl WET segments (never downloads them, and verifies bytes consumed against Content-Length); `ingestion/crawl_core.py` is the runner-independent ingest, `spark_crawl.py` and `beam_crawl.py` distribute it, and the runner test holds all three to one corpus fingerprint. Cleaning is per document, deduplication is a shuffle on content hash, and the reserve pool excludes anything already in training. **Run on the real crawl: 5,638,371 documents streamed and cleaned from 270 of 284 WET segments in 2.05 hours on 4 workers (762 documents/second), 90,948 written to the reserve pool** at `reports/experiments/crawl/spark/`. | 7 | **implemented, distributed and run at scale** |
